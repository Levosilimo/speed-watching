...
// the override log that entrypoints/content.ts (YouTube) and
// entrypoints/generic.content.ts both run. The page deltas are deps
// (lib/rate-controller-types.ts): the pill anchor, how a rate gets applied
// (direct assignment vs lib/matcher.ts's re-assert loop), the current-video
// context, and the override log's video id.
...
export function createRateController<C extends RateCurrent>(deps: RateControllerDeps<C>) {
  ...
  /** Video epoch: bumped by every reset so an in-flight measure cannot
   * render — or auto-apply — for the video it measured after the reset. */
  let epoch = 0;
  ...
  /** The session's accumulated flushes, null before apply, while paused, or
   * while the rate diverged (the tracker's accrual gates, mirrored). */
   ...
   * ('apply') from scheduler boundary steps ('adjust'). Returns the clamped
   * rate actually applied (the scheduler's read-back contract). */
  ...
  /** Re-renders only when the scheduler status changed (steps flow through
   * onMediaEvent). */
  ...
  /** The active video left the DOM: destroy the pill, drop the re-assert
   * loop and the recommendation, end the session. */
