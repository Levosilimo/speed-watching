...
| Screenshots | `docs/store-screenshot.png` (1280×800, captured by `tools/capture-screenshot.mjs`) — regenerated 2026-08-13 with the a11y palette and shipped sections, but the capture fixture still has no auto-apply card (the section landed in a parallel lane; the surface lane is reordering sections) — re-verify after the surface-lane merge and re-capture if the frame changed |
...
- [ ] Re-verify `docs/store-screenshot.png` after the surface-lane merge
